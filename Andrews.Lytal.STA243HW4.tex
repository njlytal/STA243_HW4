...
\item[b.] We show that the following statements hold:
\begin{description}
\item[i.] $(\lambda | p, \textbf{r}, \textbf{x}) \sim \text{Gamma}(a + \sum_i x_i, b+\sum_i r_i)$\\
\[p(\lambda | p, \textbf{r}, \textbf{x}) \propto p(\lambda|p) \cdot p(x_i | \textbf{r}, \lambda, p)\]
\[\propto \text{Gamma}(a, b) \cdot \prod_{i = 1}^n \text{Poisson}(\lambda r_i)\]
\[\propto \frac{b^a}{\Gamma(a)} \lambda^{a - 1} e^{-b\lambda}\cdot \prod_{i = 1}^n \frac{(\lambda r_i)^{x_i}}{x_i!}e^{-\lambda r_i}\]
\[\propto \frac{b^a}{\Gamma(a)} \lambda^{a - 1} e^{-b\lambda} \cdot \frac{\lambda^{\sum_{i=1}^n x_i} \prod_{i=1}^n (r_i^{x_i})}{\prod_{i = 1}^n x_i !} e^{-\lambda \sum_{i = 1}^n r_i} \]
\[\propto \lambda^{a - 1} e^{-b\lambda} \cdot \lambda^{\sum_{i = 1}^n x_i } e^{-\lambda \sum_{i = 1}^n r_i}\]
\[\propto \lambda^{a + \sum_{i = 1}^n x_i - 1} \cdot e^{-(b + \sum_{i = 1}^n r_i)\lambda}\]


\[\propto \Gamma \left(a + \sum_{i = 1}^n x_i, b + \sum_{i = 1}^n r_i \right) \]

\item[ii.] $(p| \lambda, \textbf{r}, \textbf{x}) \sim \text{Beta}(1 + \sum_i r_i, n + 1 - \sum_i r_i)$ \\
\[p(p| \lambda, \textbf{r}, \textbf{x}) \propto p(p) \cdot (\textbf{r} | p, \lambda) \]
\[\propto \frac{1}{1 - 0} \cdot \prod_{i = 1}^n p(r_i | p, \lambda)\]
\[\propto \prod_{i = 1}^n p^{r_i} (1 - p)^{1 - r_i}  = p^{\sum_{i = 1}^n r_i} (1 - p)^{n - \sum_{i=1}^n r_i}\]

\[\propto \text{Beta}\left (\sum_{i=1}^n r_i + 1, n - \sum_{i=1}^n r_i + 1\right )\]
